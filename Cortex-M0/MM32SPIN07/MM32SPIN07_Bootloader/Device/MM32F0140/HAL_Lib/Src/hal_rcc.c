...
static __I uint8_t tbPresc[] = {0, 0, 0, 0, 1, 2, 3, 4, 1, 2, 3, 4, 6, 7, 8, 9};
...
void RCC_GetClocksFreq(RCC_ClocksTypeDef* clk)
{
    ...
    ADCPrescH = (ADC1->CFGR & (0x07U << ADC_CFGR_PRE_Pos)) >> ADC_CFGR_PRE_Pos;
    ADCPrescL = (ADC1->CFGR & (0x01U << ADC_CFGR_PREL_Pos)) >> ADC_CFGR_PREL_Pos;
    clk->ADCCLK_Frequency = clk->PCLK2_Frequency / (((ADCPrescH << 1) | ADCPrescL) + 2);
}
