...
uint16_t I2C_Master_Write(I2C_TypeDef *I2Cx, uint16_t dev_address, uint8_t *data_p, uint16_t size, uint32_t timeout)
{
    ...
    if(size > 0)
    {
        ...
        while(1)
        {
            if(I2C_GetFlagStatus(I2Cx, I2C_IT_TX_ABRT))
            {
                //if(!(I2Cx->RESERVED0x80 & 0x000000001F))
                //{
                //    remain += 1;
                //}
                I2C_ClearFlag(I2Cx, I2C_IT_TX_ABRT);
            }
            
            if(I2C_GetFlagStatus(I2Cx, I2C_STATUS_FLAG_TFE))
            {
                I2C_SendData(I2Cx, *data_p++);
                //remain--;
                cnt++;
            }
    ...
    if(size > 0)
    {
        ...
        while(1)
        {
            if(I2C_GetFlagStatus(I2Cx, I2C_IT_TX_ABRT))
            {
                //if(!(I2Cx->RESERVED0x80 & 0x000000001F))
                //{
                //    remain += 1;//I2Cx->RESERVED0x80 >> 24;
                //}
                
                I2C_ClearFlag(I2Cx, I2C_IT_TX_ABRT);
            }
            
            //while(I2C_GetFlagStatus(I2Cx, I2C_STATUS_FLAG_TFNF) && (remain > 0))
            if(I2C_GetFlagStatus(I2Cx, I2C_STATUS_FLAG_TFE) && (remain > 0))
            {
                I2C_ReadCmd(I2Cx);
                remain--;
            }
...
void I2C_Master_Init(void)
{
	...
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource5, GPIO_AF_5); //I2C1_SCL
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource4, GPIO_AF_5); //I2C1_SDA
