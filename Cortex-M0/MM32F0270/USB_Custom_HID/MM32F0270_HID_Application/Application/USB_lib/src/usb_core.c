...
u8 Setup0_Process(void)
{
    ...
    if (pInformation->ControlState != PAUSE) {
        pInformation->USBbmRequestType = USB->SETUPx[0];  // bmRequestType
        pInformation->USBbRequest = USB->SETUPx[1];       // bRequest //
        pInformation->USBwValue = (USB->SETUPx[3]) | (USB->SETUPx[2] << 8); // wValue
        pInformation->USBwIndex  = (USB->SETUPx[5]) | (USB->SETUPx[4] << 8); // wIndex
        pInformation->USBwLength = (USB->SETUPx[6]) | (USB->SETUPx[7] << 8); // wLength
    }
