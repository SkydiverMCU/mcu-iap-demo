...
////////////////////////////////////////////////////////////////////////////////
/// @brief  Configure the GPIO Pins to pull up or down at open-drain output mode.
/// @param  gpio: to select the GPIO peripheral.
/// @param  pin: specifies the port bit.
///         This parameter can be any combination of GPIO_Pin_x where x can be
///         (0..15).
/// @param  pull_mode: set pull up or down.
...
void GPIO_PinODConfig(GPIO_TypeDef* gpio, u16 pin, u8 pull_mode)
{
    ...
    for (i = 0; i < 16; i++) {
        if (pin & 0x01) {
            MODIFY_REG(gpio->DCR, GPIO_DCR_PX0 << (i * 2), pull_mode << (i * 2));
        }
        pin >>= 1;
    }
}
